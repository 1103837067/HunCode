...
- run interactive coding agent session: `pnpm dlx @mariozechner/pi-coding-agent`
- enter `/login` command
...
Update mom anytime with `pnpm add -g @mariozechner/pi-mom`. This only updates the Node.js app on your host. Anything mom installed inside the Docker container remains unchanged.
...
```bash
cd packages/mom
pnpm exec tsx --watch-path src --watch src/main.ts --sandbox=docker:mom-sandbox ./data
```
